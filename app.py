...
with open(SORTED_INDEX_FILEPATH, "rb") as f:
    ...
    i = 0
    while sig:
        f.seek(f.tell() + sample_index_interval * SHA1_SIZE_IN_BYTES)
        sig = f.read(SHA1_SIZE_IN_BYTES)
        #print(b2a(sig))
        idx += sig
...
def get_file_position(q):
    ...
    while q != sig and size > 1:
        log.debug("get file position for query %s : current sig %s position %10s : size %s", b2a(q), b2a(sig), offset / SHA1_SIZE_IN_BYTES, size)
        size = ceil(size / 2.)
        if q > sig:
            offset += size * SHA1_SIZE_IN_BYTES
        if q < sig:
            offset -= size * SHA1_SIZE_IN_BYTES
        sig = idx[offset:offset + SHA1_SIZE_IN_BYTES]
    log.debug("result for file position for query %s : current position %10s : size %s : %s", b2a(q), offset / SHA1_SIZE_IN_BYTES, size, q != sig)
    return int(offset / SHA1_SIZE_IN_BYTES)


def is_present(q):
    offset = get_file_position(q) * sample_index_interval * SHA1_SIZE_IN_BYTES
    print("offset %s" % offset)
    with open(SORTED_INDEX_FILEPATH, "rb") as f:
        size = sample_index_interval * 2
        f.seek(offset)
        sig = f.read(SHA1_SIZE_IN_BYTES)
        log.debug("query : %s / current %s", b2a(q), b2a(sig))
        while q != sig and size > 1:
